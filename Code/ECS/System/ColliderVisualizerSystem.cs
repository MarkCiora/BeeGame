...
public class ColliderVisualizerSystem : ECSSystem
{
    public void Update()
    {
        ...
        foreach (var entity in m_entities)
        {
            ...
            var collider = ecs.GetComponent<CircleCollider>(entity);
            var tex = Textures.white_circle;
            Vector2 screen_pos = camera.WorldToScreen(transform.pos);
            Rectangle rect = new(0, 0, tex.Height, tex.Height);
            float scale = 2 * collider.radius * camera.zoom / tex.Height;
